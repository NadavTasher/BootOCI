...
def oci_to_bootable(
    *,
    ...
    if rootfs_from_dockerfile:
        ...
        if not subprocess.run(backend + ["image", "ls", "--quiet", rootfs_from_tag], capture_output=True, check=True).stdout:
            # Pull the image
            subprocess.run(backend + ["pull", rootfs_from_tag], check=True)
